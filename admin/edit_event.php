...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Calculate available tickets based on new total
    ...
    if ($new_available_tickets < 0) {
        $error = "Cannot reduce tickets below already booked count ($booked_tickets tickets booked)";
    } elseif ($price < 0) {
        ...
        try {
            $stmt = $pdo->prepare("
                UPDATE events 
                SET name = ?, description = ?, event_date = ?, venue = ?, 
            ...
            $stmt->execute([
                ...
                $total_tickets, $new_available_tickets, $price,
...
<head>
    ...
    <title>Edit Event - U-Book Admin</title>
        ...
        <form method="POST" class="event-form">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="total_tickets" name="total_tickets" min="1" 
                           value="<?php echo $event['total_tickets']; ?>" required>
                    <small>Already booked: <?php echo $event['total_tickets'] - $event['available_tickets']; ?> tickets</small>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="price-options">
                    ...
                    $is_custom = $current_price > 0 && $current_price != 10 && $current_price != 25;
                    ?>
                    
                    <label class="price-option <?php echo $current_price == 0 ? 'selected' : ''; ?>" 
                           onclick="selectPrice(0)">
                        <input type="radio" name="price" value="0" <?php echo $current_price == 0 ? 'checked' : ''; ?>>
                        <div class="price-display">FREE</div>
                        <div class="price-label">No charge</div>
                    </label>
                    
                    <label class="price-option <?php echo $current_price == 10 ? 'selected' : ''; ?>" 
                           onclick="selectPrice(10)">
                        <input type="radio" name="price" value="10" <?php echo $current_price == 10 ? 'checked' : ''; ?>>
                        <div class="price-display">$10</div>
                        <div class="price-label">Budget friendly</div>
                    </label>
                    
                    <label class="price-option <?php echo $current_price == 25 ? 'selected' : ''; ?>" 
                           onclick="selectPrice(25)">
                        <input type="radio" name="price" value="25" <?php echo $current_price == 25 ? 'checked' : ''; ?>>
                        <div class="price-display">$25</div>
                        <div class="price-label">Standard</div>
                    </label>
                    
                    <label class="price-option <?php echo $is_custom ? 'selected' : ''; ?>" 
                           onclick="document.getElementById('custom-price').style.display='block';">
                        <input type="radio" name="price" value="custom" <?php echo $is_custom ? 'checked' : ''; ?>>
                        <div class="price-display">Custom</div>
                        <div class="price-label">Set your price</div>
                    </label>
                </div>
                
                <div id="custom-price" style="display: <?php echo $is_custom ? 'block' : 'none'; ?>;">
                    <input type="number" name="price_custom" id="price_custom" 
                           value="<?php echo $is_custom ? $current_price : ''; ?>" 
